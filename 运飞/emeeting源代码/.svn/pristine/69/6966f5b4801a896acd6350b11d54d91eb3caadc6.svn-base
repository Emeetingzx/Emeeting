...
- (void)_addNamespace:(DDXMLNode *)namespaceNode
{
	// NSXML version uses this same assertion
	DDXMLAssert([namespaceNode _hasParent] == NO, @"Cannot add a namespace with a parent; detach or copy first");
	DDXMLAssert(IsXmlNsPtr(namespaceNode->genericPtr), @"Not a namespace");
	
	xmlNodePtr node = (xmlNodePtr)genericPtr;
	xmlNsPtr ns = (xmlNsPtr)namespaceNode->genericPtr;
	
	// Beware: [namespace prefix] does NOT return what you might expect.  Use [namespace name] instead.
	
	NSString *namespaceName = [namespaceNode name];
	...
	namespaceNode->owner = self;
	
	if ([namespaceNode isKindOfClass:[DDXMLNamespaceNode class]])
	{
		DDXMLNamespaceNode *ddNamespace = (DDXMLNamespaceNode *)namespaceNode;
...
- (void)addNamespace:(DDXMLNode *)namespaceNode
{
#if DDXML_DEBUG_MEMORY_ISSUES
	DDXMLNotZombieAssert();
#endif
	
	[self _addNamespace:namespaceNode];
}
...
- (void)setNamespaces:(NSArray *)namespacesArray
{
#if DDXML_DEBUG_MEMORY_ISSUES
	DDXMLNotZombieAssert();
#endif
	...
	NSUInteger i;
	for (i = 0; i < [namespacesArray count]; i++)
	{
		DDXMLNode *namespaceNode = [namespacesArray objectAtIndex:i];
		[self _addNamespace:namespaceNode];
