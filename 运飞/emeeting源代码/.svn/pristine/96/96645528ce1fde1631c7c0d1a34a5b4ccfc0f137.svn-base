...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    {
        NSString *pinyin = [((PinYinString*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((PinYinString*)object).pinYin substringToIndex:1];
        NSString *string = ((PinYinString*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    {
        PinYinString *chineseString = [[PinYinString alloc]init];
        ...
        chineseString.string = [PinYinString RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        [result addObject:((PinYinString*)[tempArray objectAtIndex:i]).book];
        NSLog(@"SortArray----->%@",((PinYinString*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
