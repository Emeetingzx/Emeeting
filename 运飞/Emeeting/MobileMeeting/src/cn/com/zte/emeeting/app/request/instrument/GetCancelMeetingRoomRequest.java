 ...
 * 
 */
public class GetCancelMeetingRoomRequest extends AppHttpRequest {
	 ...
	 *            会议ID
	 * @param RoomId
	 * 会议室ID 　　输入会议室ID，服务端会退订对应会议下对应会议室。 　　
	 * 不输入会议室ID，服务端会退订对应回一下所有会议室。
	 */
	public GetCancelMeetingRoomRequest(Context context, boolean httpsFlag,
			String EmeetingId, String RoomId) {
		super(context, httpsFlag);
		setWebServiceMethod(HttpUtil.PublicInterface);
		setC(HttpUtil.CancelMeetingRoom);
	 ...
	 * 不输入会议室ID，服务端会退订对应回一下所有会议室。
	 */
	public GetCancelMeetingRoomRequest(Context context, String EmeetingId,
			String RoomId) {
